...
-   Query builder: Cung cấp một giao diện tiên lợi để thực thi các câu truy vấn SQL. Laravel query builder sử dụng **PDO parameter binding** để bảo vệ ứng dụng khỏi các cuộc tấn công **SQL injection**. PDO không hỗ trợ binding cho các tên bảng và cột vì SQL engine cần phải biết tên bảng và cột ngay lúc thực thi câu lệnh.
-   Eloquent ORM: Eloquent ORM là một ORM mạnh mẽ trong Laravel. Nó cung cấp một cách tiện lợi để tương tác với cơ sở dữ liệu. Mỗi bảng trong cơ sở dữ liệu tương ứng với một model trong Eloquent, được sử dụng để thao tác với bảng đó. Eloquent cung cấp một cách tiện lợi để tạo, truy vấn, cập nhật và xóa dữ liệu từ cơ sở dữ liệu.

2. Nêu ưu nhược điểm của Eloquent ORM và Query builder

-   Query builder:

    -   Ưu điểm:
        -   Dễ học và sử dụng
        -   Cung cấp một cách tiện lợi để thực thi các câu truy vấn SQL
        -   Bảo vệ ứng dụng khỏi các cuộc tấn công SQL injection
    -   Nhược điểm:
        -   Không hỗ trợ các tính năng của Eloquent như quan hệ, accessors, mutators, ...

-   Eloquent ORM:
    -   Ưu điểm:
        -   Cung cấp một cách tiện lợi để tương tác với cơ sở dữ liệu
        -   Cung cấp các tính năng như quan hệ, accessors, mutators, ...
        -   Dễ dàng tạo, truy vấn, cập nhật và xóa dữ liệu từ cơ sở dữ liệu
    -   Nhược điểm:
        -   Không thể thực thi các câu truy vấn phức tạp như Query builder
        -   Hiệu suất không cao bằng Query builder
        -   Không thể tùy chỉnh câu truy vấn SQL
        -   Không hỗ trợ các tính năng của Query builder như PDO parameter binding

3. Khi nào nên sử dụng Eloquent ORM và Query builder?

-   Sử dụng Eloquent ORM khi cần tương tác với cơ sở dữ liệu một cách tiện lợi và nhanh chóng, và không cần thực thi các câu truy vấn SQL phức tạp.
-   Sử dụng Query builder khi cần thực thi các câu truy vấn SQL phức tạp, cần tùy chỉnh câu truy vấn SQL, hoặc cần hiệu suất cao hơn so với Eloquent ORM.

4. Phân biệt lazy loading và eager loading

-   Lazy loading: dữ liệu quan hệ không thật sự được truy vấn cho đến khi bạn truy cập vào nó. Lazy loading có thể dẫn đến vấn đề N + 1, khi mỗi bản ghi cha được truy cập, một truy vấn riêng lẻ được thực thi để truy cập dữ liệu quan hệ.
-   Eager loading: dữ liệu quan hệ được truy vấn cùng với bản ghi cha. Eager loading giúp tránh vấn đề N + 1 bằng cách thực thi một truy vấn duy nhất để truy cập dữ liệu quan hệ.

5. Phân biệt with() và load()
-  `with()`: Eager loading dữ liệu quan hệ khi truy vấn dữ liệu
-  `load()`: Eager loading dữ liệu quan hệ sau khi truy vấn dữ liệu. Có ích khi cần eager loading dữ liệu sau một điều kiện nào đó.
